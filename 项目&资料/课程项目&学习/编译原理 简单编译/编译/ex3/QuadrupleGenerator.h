...
struct Quadruple {
    ...
    static void printQuadruples(const vector<Quadruple>& quads) {
        int idx = 1;
        for (const auto& q : quads) {
            cout << idx++ << " (" << q.op << ", " << q.arg1 << ", " << q.arg2 << ", " << q.result << ");" << endl;
        }
    }
     //遍历四元式向量，并按顺序输出每个四元式，同时为每个四元式添加行号
    // 解析标签,将label替换为行号 用于解析四元式中的标签，将标签替换为实际的行号
